			...
			"Option" => {
				...
				if let Some(inner_path) = contained_struct {
					if self.c_type_has_inner_from_path(&inner_path) {
						...
						return Some(("if ", vec![
							(format!(".is_none() {{ {}::COption_{}Z::None }} else {{ ", Self::generated_container_path(), inner_name),
							 format!("{}::COption_{}Z::Some({}.unwrap())", Self::generated_container_path(), inner_name, var_access))
			...
			"Option" => {
				if let Some(syn::Type::Path(p)) = single_contained {
					...
					if self.is_primitive(&inner_path) {
						...
						if is_ref {
							return Some(("if ", vec![(".inner.is_null() { None } else {/*B*/ Some((*".to_string(), format!("{}", var_access))], ").clone()) }", ContainerPrefixLocation::PerConv))
						} else {
							return Some(("if ", vec![(".inner.is_null() { None } else {/*A*/ Some(".to_string(), format!("{}", var_access))], ") }", ContainerPrefixLocation::PerConv));
						}
					}
				}
						...
						syn::Type::Reference(_)|syn::Type::Path(_)|syn::Type::Slice(_) => {
							...
							match ret_ref {
								...
								EmptyValExpectedTy::OptionType =>
									...
									], ".take()) }", ContainerPrefixLocation::NoPrefix)),
