...
pub enum PendingHTLCRouting {
	...
	Forward {
		...
		hold_htlc: crate::c_types::derived::COption_NoneZ,
	},
	...
	TrampolineForward {
		...
		incoming_shared_secret: crate::c_types::ThirtyTwoBytes,
		/// The onion which should be included in the forwarded HTLC, telling the next hop what to
		/// do with the HTLC.
		onion_packet: crate::lightning::ln::msgs::TrampolineOnionPacket,
...
impl PendingHTLCRouting {
	...
	pub(crate) fn to_native(&self) -> nativePendingHTLCRouting {
		match self {
			PendingHTLCRouting::Forward {ref onion_packet, ref short_channel_id, ref blinded, ref incoming_cltv_expiry, ref hold_htlc, } => {
				...
				let mut hold_htlc_nonref = Clone::clone(hold_htlc);
				let mut local_hold_htlc_nonref = if hold_htlc_nonref.is_some() { Some( { () /*hold_htlc_nonref.take()*/ }) } else { None };
				nativePendingHTLCRouting::Forward {
					...
					hold_htlc: local_hold_htlc_nonref,
				}
			},
			PendingHTLCRouting::TrampolineForward {ref incoming_shared_secret, ref onion_packet, ref node_id, ref blinded, ref incoming_cltv_expiry, } => {
				let mut incoming_shared_secret_nonref = Clone::clone(incoming_shared_secret);
				let mut onion_packet_nonref = Clone::clone(onion_packet);
				let mut node_id_nonref = Clone::clone(node_id);
				let mut blinded_nonref = Clone::clone(blinded);
				let mut local_blinded_nonref = if blinded_nonref.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(blinded_nonref.take_inner()) } }) };
				let mut incoming_cltv_expiry_nonref = Clone::clone(incoming_cltv_expiry);
				nativePendingHTLCRouting::TrampolineForward {
					incoming_shared_secret: incoming_shared_secret_nonref.data,
					onion_packet: *unsafe { Box::from_raw(onion_packet_nonref.take_inner()) },
			...
			PendingHTLCRouting::ReceiveKeysend {ref payment_data, ref payment_preimage, ref payment_metadata, ref incoming_cltv_expiry, ref custom_tlvs, ref requires_blinded_error, ref has_recipient_created_payment_secret, ref invoice_request, ref payment_context, } => {
				let mut payment_data_nonref = Clone::clone(payment_data);
				...
				let mut local_invoice_request_nonref = if invoice_request_nonref.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(invoice_request_nonref.take_inner()) } }) };
				let mut payment_context_nonref = Clone::clone(payment_context);
				let mut local_payment_context_nonref = { /*payment_context_nonref*/ let payment_context_nonref_opt = payment_context_nonref; if payment_context_nonref_opt.is_none() { None } else { Some({ { { payment_context_nonref_opt.take() }.into_native() }})} };
				nativePendingHTLCRouting::ReceiveKeysend {
					...
					invoice_request: local_invoice_request_nonref,
					payment_context: local_payment_context_nonref,
				}
			},
		}
	...
	pub(crate) fn into_native(self) -> nativePendingHTLCRouting {
		match self {
			PendingHTLCRouting::Forward {mut onion_packet, mut short_channel_id, mut blinded, mut incoming_cltv_expiry, mut hold_htlc, } => {
				...
				let mut local_hold_htlc = if hold_htlc.is_some() { Some( { () /*hold_htlc.take()*/ }) } else { None };
				nativePendingHTLCRouting::Forward {
					...
					hold_htlc: local_hold_htlc,
				}
			},
			PendingHTLCRouting::TrampolineForward {mut incoming_shared_secret, mut onion_packet, mut node_id, mut blinded, mut incoming_cltv_expiry, } => {
				let mut local_blinded = if blinded.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(blinded.take_inner()) } }) };
				nativePendingHTLCRouting::TrampolineForward {
					incoming_shared_secret: incoming_shared_secret.data,
					onion_packet: *unsafe { Box::from_raw(onion_packet.take_inner()) },
			...
			PendingHTLCRouting::ReceiveKeysend {mut payment_data, mut payment_preimage, mut payment_metadata, mut incoming_cltv_expiry, mut custom_tlvs, mut requires_blinded_error, mut has_recipient_created_payment_secret, mut invoice_request, mut payment_context, } => {
				let mut local_payment_data = if payment_data.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(payment_data.take_inner()) } }) };
				let mut local_payment_metadata = { /*payment_metadata*/ let payment_metadata_opt = payment_metadata; if payment_metadata_opt.is_none() { None } else { Some({ { let mut local_payment_metadata_0 = Vec::new(); for mut item in { payment_metadata_opt.take() }.into_rust().drain(..) { local_payment_metadata_0.push( { item }); }; local_payment_metadata_0 }})} };
				let mut local_custom_tlvs = Vec::new(); for mut item in custom_tlvs.into_rust().drain(..) { local_custom_tlvs.push( { let (mut orig_custom_tlvs_0_0, mut orig_custom_tlvs_0_1) = item.to_rust(); let mut local_orig_custom_tlvs_0_1 = Vec::new(); for mut item in orig_custom_tlvs_0_1.into_rust().drain(..) { local_orig_custom_tlvs_0_1.push( { item }); }; let mut local_custom_tlvs_0 = (orig_custom_tlvs_0_0, local_orig_custom_tlvs_0_1); local_custom_tlvs_0 }); };
				let mut local_invoice_request = if invoice_request.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(invoice_request.take_inner()) } }) };
				let mut local_payment_context = { /*payment_context*/ let payment_context_opt = payment_context; if payment_context_opt.is_none() { None } else { Some({ { { payment_context_opt.take() }.into_native() }})} };
				nativePendingHTLCRouting::ReceiveKeysend {
					...
					invoice_request: local_invoice_request,
					payment_context: local_payment_context,
				}
			},
		}
	...
	pub(crate) fn from_native(native: &PendingHTLCRoutingImport) -> Self {
		...
		match native {
			nativePendingHTLCRouting::Forward {ref onion_packet, ref short_channel_id, ref blinded, ref incoming_cltv_expiry, ref hold_htlc, } => {
				let mut onion_packet_nonref = Clone::clone(onion_packet);
				let mut short_channel_id_nonref = Clone::clone(short_channel_id);
				let mut blinded_nonref = Clone::clone(blinded);
				...
				let mut hold_htlc_nonref = Clone::clone(hold_htlc);
				let mut local_hold_htlc_nonref = if hold_htlc_nonref.is_none() { crate::c_types::derived::COption_NoneZ::None } else { crate::c_types::derived::COption_NoneZ::Some /*  { () /**/ } */ };
			...
			nativePendingHTLCRouting::TrampolineForward {ref incoming_shared_secret, ref onion_packet, ref node_id, ref blinded, ref incoming_cltv_expiry, } => {
				let mut incoming_shared_secret_nonref = Clone::clone(incoming_shared_secret);
				let mut onion_packet_nonref = Clone::clone(onion_packet);
				let mut node_id_nonref = Clone::clone(node_id);
				let mut blinded_nonref = Clone::clone(blinded);
				...
				PendingHTLCRouting::TrampolineForward {
					incoming_shared_secret: crate::c_types::ThirtyTwoBytes { data: incoming_shared_secret_nonref },
					onion_packet: crate::lightning::ln::msgs::TrampolineOnionPacket { inner: ObjOps::heap_alloc(onion_packet_nonref), is_owned: true },
			...
			nativePendingHTLCRouting::ReceiveKeysend {ref payment_data, ref payment_preimage, ref payment_metadata, ref incoming_cltv_expiry, ref custom_tlvs, ref requires_blinded_error, ref has_recipient_created_payment_secret, ref invoice_request, ref payment_context, } => {
				let mut payment_data_nonref = Clone::clone(payment_data);
				...
				let mut has_recipient_created_payment_secret_nonref = Clone::clone(has_recipient_created_payment_secret);
				let mut invoice_request_nonref = Clone::clone(invoice_request);
				let mut local_invoice_request_nonref = crate::lightning::offers::invoice_request::InvoiceRequest { inner: if invoice_request_nonref.is_none() { core::ptr::null_mut() } else {  { ObjOps::heap_alloc((invoice_request_nonref.unwrap())) } }, is_owned: true };
				let mut payment_context_nonref = Clone::clone(payment_context);
				let mut local_payment_context_nonref = if payment_context_nonref.is_none() { crate::c_types::derived::COption_PaymentContextZ::None } else { crate::c_types::derived::COption_PaymentContextZ::Some( { crate::lightning::blinded_path::payment::PaymentContext::native_into(payment_context_nonref.unwrap()) }) };
	...
	pub(crate) fn native_into(native: nativePendingHTLCRouting) -> Self {
		match native {
			nativePendingHTLCRouting::Forward {mut onion_packet, mut short_channel_id, mut blinded, mut incoming_cltv_expiry, mut hold_htlc, } => {
				...
				let mut local_hold_htlc = if hold_htlc.is_none() { crate::c_types::derived::COption_NoneZ::None } else { crate::c_types::derived::COption_NoneZ::Some /*  { () /**/ } */ };
				PendingHTLCRouting::Forward {
			...
			nativePendingHTLCRouting::TrampolineForward {mut incoming_shared_secret, mut onion_packet, mut node_id, mut blinded, mut incoming_cltv_expiry, } => {
				let mut local_blinded = crate::lightning::ln::channelmanager::BlindedForward { inner: if blinded.is_none() { core::ptr::null_mut() } else {  { ObjOps::heap_alloc((blinded.unwrap())) } }, is_owned: true };
				PendingHTLCRouting::TrampolineForward {
					incoming_shared_secret: crate::c_types::ThirtyTwoBytes { data: incoming_shared_secret },
					onion_packet: crate::lightning::ln::msgs::TrampolineOnionPacket { inner: ObjOps::heap_alloc(onion_packet), is_owned: true },
			...
			nativePendingHTLCRouting::ReceiveKeysend {mut payment_data, mut payment_preimage, mut payment_metadata, mut incoming_cltv_expiry, mut custom_tlvs, mut requires_blinded_error, mut has_recipient_created_payment_secret, mut invoice_request, mut payment_context, } => {
				...
				let mut local_payment_metadata = if payment_metadata.is_none() { crate::c_types::derived::COption_CVec_u8ZZ::None } else { crate::c_types::derived::COption_CVec_u8ZZ::Some( { let mut local_payment_metadata_0 = Vec::new(); for mut item in payment_metadata.unwrap().drain(..) { local_payment_metadata_0.push( { item }); }; local_payment_metadata_0.into() }) };
				let mut local_custom_tlvs = Vec::new(); for mut item in custom_tlvs.drain(..) { local_custom_tlvs.push( { let (mut orig_custom_tlvs_0_0, mut orig_custom_tlvs_0_1) = item; let mut local_orig_custom_tlvs_0_1 = Vec::new(); for mut item in orig_custom_tlvs_0_1.drain(..) { local_orig_custom_tlvs_0_1.push( { item }); }; let mut local_custom_tlvs_0 = (orig_custom_tlvs_0_0, local_orig_custom_tlvs_0_1.into()).into(); local_custom_tlvs_0 }); };
				let mut local_invoice_request = crate::lightning::offers::invoice_request::InvoiceRequest { inner: if invoice_request.is_none() { core::ptr::null_mut() } else {  { ObjOps::heap_alloc((invoice_request.unwrap())) } }, is_owned: true };
				let mut local_payment_context = if payment_context.is_none() { crate::c_types::derived::COption_PaymentContextZ::None } else { crate::c_types::derived::COption_PaymentContextZ::Some( { crate::lightning::blinded_path::payment::PaymentContext::native_into(payment_context.unwrap()) }) };
				PendingHTLCRouting::ReceiveKeysend {
					payment_data: local_payment_data,
...
pub extern "C" fn PendingHTLCRouting_forward(onion_packet: crate::lightning::ln::msgs::OnionPacket, short_channel_id: u64, blinded: crate::lightning::ln::channelmanager::BlindedForward, incoming_cltv_expiry: crate::c_types::derived::COption_u32Z, hold_htlc: crate::c_types::derived::COption_NoneZ) -> PendingHTLCRouting {
	PendingHTLCRouting::Forward {
		onion_packet,
		short_channel_id,
		blinded,
...
pub extern "C" fn PendingHTLCRouting_trampoline_forward(incoming_shared_secret: crate::c_types::ThirtyTwoBytes, onion_packet: crate::lightning::ln::msgs::TrampolineOnionPacket, node_id: crate::c_types::PublicKey, blinded: crate::lightning::ln::channelmanager::BlindedForward, incoming_cltv_expiry: u32) -> PendingHTLCRouting {
	PendingHTLCRouting::TrampolineForward {
...
pub extern "C" fn PendingHTLCRouting_receive_keysend(payment_data: crate::lightning::ln::msgs::FinalOnionHopData, payment_preimage: crate::c_types::ThirtyTwoBytes, payment_metadata: crate::c_types::derived::COption_CVec_u8ZZ, incoming_cltv_expiry: u32, custom_tlvs: crate::c_types::derived::CVec_C2Tuple_u64CVec_u8ZZZ, requires_blinded_error: bool, has_recipient_created_payment_secret: bool, invoice_request: crate::lightning::offers::invoice_request::InvoiceRequest, payment_context: crate::c_types::derived::COption_PaymentContextZ) -> PendingHTLCRouting {
	PendingHTLCRouting::ReceiveKeysend {
		payment_data,
		payment_preimage,
		payment_metadata,
...
pub extern "C" fn BlindedForward_get_inbound_blinding_point(this_ptr: &BlindedForward) -> crate::c_types::PublicKey {
	let mut inner_val = &mut BlindedForward::get_native_mut_ref(this_ptr).inbound_blinding_point;
...
pub extern "C" fn BlindedForward_get_failure(this_ptr: &BlindedForward) -> crate::lightning::ln::channelmanager::BlindedFailure {
	let mut inner_val = &mut BlindedForward::get_native_mut_ref(this_ptr).failure;
...
pub extern "C" fn BlindedForward_get_next_blinding_override(this_ptr: &BlindedForward) -> crate::c_types::PublicKey {
	let mut inner_val = &mut BlindedForward::get_native_mut_ref(this_ptr).next_blinding_override;
...
pub(crate) extern "C" fn BlindedForward_clone_void(this_ptr: *const c_void) -> *mut c_void {
	Box::into_raw(Box::new(Clone::clone(unsafe { &*(this_ptr as *const nativeBlindedForward) }))) as *mut c_void
}
#[no_mangle]
...
pub extern "C" fn PendingHTLCInfo_get_routing(this_ptr: &PendingHTLCInfo) -> crate::lightning::ln::channelmanager::PendingHTLCRouting {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).routing;
...
pub extern "C" fn PendingHTLCInfo_get_incoming_shared_secret(this_ptr: &PendingHTLCInfo) -> *const [u8; 32] {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).incoming_shared_secret;
	inner_val
}
/// The onion shared secret we build with the sender used to decrypt the onion.
///
...
pub extern "C" fn PendingHTLCInfo_get_payment_hash(this_ptr: &PendingHTLCInfo) -> *const [u8; 32] {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).payment_hash;
	&inner_val.0
}
/// Hash of the payment preimage, to lock the payment until the receiver releases the preimage.
#[no_mangle]
...
pub extern "C" fn PendingHTLCInfo_get_incoming_amt_msat(this_ptr: &PendingHTLCInfo) -> crate::c_types::derived::COption_u64Z {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).incoming_amt_msat;
...
pub extern "C" fn PendingHTLCInfo_get_outgoing_amt_msat(this_ptr: &PendingHTLCInfo) -> u64 {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).outgoing_amt_msat;
	*inner_val
}
/// The amount the sender indicated should be forwarded on to the next hop or amount the sender
/// intended for us to receive for received payments.
...
pub extern "C" fn PendingHTLCInfo_get_outgoing_cltv_value(this_ptr: &PendingHTLCInfo) -> u32 {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).outgoing_cltv_value;
	*inner_val
}
/// The CLTV the sender has indicated we should set on the forwarded HTLC (or has indicated
/// should have been set on the received HTLC for received payments).
...
pub extern "C" fn PendingHTLCInfo_get_skimmed_fee_msat(this_ptr: &PendingHTLCInfo) -> crate::c_types::derived::COption_u64Z {
	let mut inner_val = &mut PendingHTLCInfo::get_native_mut_ref(this_ptr).skimmed_fee_msat;
	let mut local_inner_val = if inner_val.is_none() { crate::c_types::derived::COption_u64Z::None } else { crate::c_types::derived::COption_u64Z::Some( { inner_val.unwrap() }) };
	local_inner_val
}
/// The fee taken for this HTLC in addition to the standard protocol HTLC fees.
...
pub(crate) extern "C" fn PendingHTLCInfo_clone_void(this_ptr: *const c_void) -> *mut c_void {
	Box::into_raw(Box::new(Clone::clone(unsafe { &*(this_ptr as *const nativePendingHTLCInfo) }))) as *mut c_void
}
#[no_mangle]
...
use lightning::ln::channelmanager::OptionalOfferPaymentParams as nativeOptionalOfferPaymentParamsImport;
pub(crate) type nativeOptionalOfferPaymentParams = nativeOptionalOfferPaymentParamsImport;
...
pub struct OptionalOfferPaymentParams {
	...
	/// Nearly everywhere, inner must be non-null, however in places where
	/// the Rust equivalent takes an Option, it may be set to null to indicate None.
	pub inner: *mut nativeOptionalOfferPaymentParams,
	/// Indicates that this is the only struct which contains the same pointer.
...
impl core::ops::Deref for OptionalOfferPaymentParams {
	type Target = nativeOptionalOfferPaymentParams;
	fn deref(&self) -> &Self::Target { unsafe { &*ObjOps::untweak_ptr(self.inner) } }
}
unsafe impl core::marker::Send for OptionalOfferPaymentParams { }
unsafe impl core::marker::Sync for OptionalOfferPaymentParams { }
impl Drop for OptionalOfferPaymentParams {
	fn drop(&mut self) {
		if self.is_owned && !<*mut nativeOptionalOfferPaymentParams>::is_null(self.inner) {
			let _ = unsafe { Box::from_raw(ObjOps::untweak_ptr(self.inner)) };
		}
	}
}
/// Frees any resources used by the OptionalOfferPaymentParams, if is_owned is set and inner is non-NULL.
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_free(this_obj: OptionalOfferPaymentParams) { }
#[allow(unused)]
/// Used only if an object of this type is returned as a trait impl by a method
pub(crate) extern "C" fn OptionalOfferPaymentParams_free_void(this_ptr: *mut c_void) {
	let _ = unsafe { Box::from_raw(this_ptr as *mut nativeOptionalOfferPaymentParams) };
}
#[allow(unused)]
impl OptionalOfferPaymentParams {
	pub(crate) fn get_native_ref(&self) -> &'static nativeOptionalOfferPaymentParams {
		unsafe { &*ObjOps::untweak_ptr(self.inner) }
	}
	pub(crate) fn get_native_mut_ref(&self) -> &'static mut nativeOptionalOfferPaymentParams {
		unsafe { &mut *ObjOps::untweak_ptr(self.inner) }
	}
	/// When moving out of the pointer, we have to ensure we aren't a reference, this makes that easy
	pub(crate) fn take_inner(mut self) -> *mut nativeOptionalOfferPaymentParams {
		assert!(self.is_owned);
		let ret = ObjOps::untweak_ptr(self.inner);
		self.inner = core::ptr::null_mut();
		ret
...
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_get_payer_note(this_ptr: &OptionalOfferPaymentParams) -> crate::c_types::derived::COption_StrZ {
	let mut inner_val = &mut OptionalOfferPaymentParams::get_native_mut_ref(this_ptr).payer_note;
...
/// [`InvoiceRequest::payer_note`].
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_set_payer_note(this_ptr: &mut OptionalOfferPaymentParams, mut val: crate::c_types::derived::COption_StrZ) {
	let mut local_val = { /*val*/ let val_opt = val; if val_opt.is_none() { None } else { Some({ { { val_opt.take() }.into_string() }})} };
	unsafe { &mut *ObjOps::untweak_ptr(this_ptr.inner) }.payer_note = local_val;
}
/// Pathfinding options which tweak how the path is constructed to the recipient.
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_get_route_params_config(this_ptr: &OptionalOfferPaymentParams) -> crate::lightning::routing::router::RouteParametersConfig {
	let mut inner_val = &mut OptionalOfferPaymentParams::get_native_mut_ref(this_ptr).route_params_config;
	crate::lightning::routing::router::RouteParametersConfig { inner: unsafe { ObjOps::nonnull_ptr_to_inner((inner_val as *const lightning::routing::router::RouteParametersConfig<>) as *mut _) }, is_owned: false }
}
/// Pathfinding options which tweak how the path is constructed to the recipient.
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_set_route_params_config(this_ptr: &mut OptionalOfferPaymentParams, mut val: crate::lightning::routing::router::RouteParametersConfig) {
	unsafe { &mut *ObjOps::untweak_ptr(this_ptr.inner) }.route_params_config = *unsafe { Box::from_raw(val.take_inner()) };
}
...
/// [`Event::PaymentFailed`]).
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_get_retry_strategy(this_ptr: &OptionalOfferPaymentParams) -> crate::lightning::ln::outbound_payment::Retry {
	let mut inner_val = &mut OptionalOfferPaymentParams::get_native_mut_ref(this_ptr).retry_strategy;
	crate::lightning::ln::outbound_payment::Retry::from_native(inner_val)
}
/// The number of tries or time during which we'll retry this payment if some paths to the
/// recipient fail.
///
...
/// [`Event::PaymentFailed`]).
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_set_retry_strategy(this_ptr: &mut OptionalOfferPaymentParams, mut val: crate::lightning::ln::outbound_payment::Retry) {
	unsafe { &mut *ObjOps::untweak_ptr(this_ptr.inner) }.retry_strategy = val.into_native();
}
/// Constructs a new OptionalOfferPaymentParams given each field
#[must_use]
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_new(mut payer_note_arg: crate::c_types::derived::COption_StrZ, mut route_params_config_arg: crate::lightning::routing::router::RouteParametersConfig, mut retry_strategy_arg: crate::lightning::ln::outbound_payment::Retry) -> OptionalOfferPaymentParams {
	let mut local_payer_note_arg = { /*payer_note_arg*/ let payer_note_arg_opt = payer_note_arg; if payer_note_arg_opt.is_none() { None } else { Some({ { { payer_note_arg_opt.take() }.into_string() }})} };
	OptionalOfferPaymentParams { inner: ObjOps::heap_alloc(nativeOptionalOfferPaymentParams {
		payer_note: local_payer_note_arg,
		route_params_config: *unsafe { Box::from_raw(route_params_config_arg.take_inner()) },
		retry_strategy: retry_strategy_arg.into_native(),
	}), is_owned: true }
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn OptionalOfferPaymentParams_default() -> OptionalOfferPaymentParams {
	OptionalOfferPaymentParams { inner: ObjOps::heap_alloc(Default::default()), is_owned: true }
}
...
pub extern "C" fn ChainParameters_get_network(this_ptr: &ChainParameters) -> crate::bitcoin::network::Network {
	let mut inner_val = &mut ChainParameters::get_native_mut_ref(this_ptr).network;
...
pub extern "C" fn ChainParameters_get_best_block(this_ptr: &ChainParameters) -> crate::lightning::chain::BestBlock {
	let mut inner_val = &mut ChainParameters::get_native_mut_ref(this_ptr).best_block;
...
pub(crate) extern "C" fn ChainParameters_clone_void(this_ptr: *const c_void) -> *mut c_void {
	Box::into_raw(Box::new(Clone::clone(unsafe { &*(this_ptr as *const nativeChainParameters) }))) as *mut c_void
}
#[no_mangle]
...
pub extern "C" fn PhantomRouteHints_get_channels(this_ptr: &PhantomRouteHints) -> crate::c_types::derived::CVec_ChannelDetailsZ {
	let mut inner_val = &mut PhantomRouteHints::get_native_mut_ref(this_ptr).channels;
...
pub extern "C" fn PhantomRouteHints_get_phantom_scid(this_ptr: &PhantomRouteHints) -> u64 {
	let mut inner_val = &mut PhantomRouteHints::get_native_mut_ref(this_ptr).phantom_scid;
	*inner_val
}
/// A fake scid used for representing the phantom node's fake channel in generating the invoice
/// route hints.
...
pub extern "C" fn PhantomRouteHints_get_real_node_pubkey(this_ptr: &PhantomRouteHints) -> crate::c_types::PublicKey {
	let mut inner_val = &mut PhantomRouteHints::get_native_mut_ref(this_ptr).real_node_pubkey;
	crate::c_types::PublicKey::from_rust(&inner_val)
}
/// The pubkey of the real backing node that would ultimately receive the payment.
#[no_mangle]
...
pub(crate) extern "C" fn PhantomRouteHints_clone_void(this_ptr: *const c_void) -> *mut c_void {
	Box::into_raw(Box::new(Clone::clone(unsafe { &*(this_ptr as *const nativePhantomRouteHints) }))) as *mut c_void
}
#[no_mangle]
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_get_current_config(this_arg: &crate::lightning::ln::channelmanager::ChannelManager) -> crate::lightning::util::config::UserConfig {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.get_current_config();
	crate::lightning::util::config::UserConfig { inner: ObjOps::heap_alloc(ret), is_owned: true }
}

/// Updates the current [`UserConfig`] which controls some global behavior and includes the
/// default configuration applied to all new channels.
#[no_mangle]
pub extern "C" fn ChannelManager_set_current_config(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut new_config: crate::lightning::util::config::UserConfig) {
	unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.set_current_config(*unsafe { Box::from_raw(new_config.take_inner()) })
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_pay_for_bolt11_invoice(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, invoice: &crate::lightning_invoice::Bolt11Invoice, mut payment_id: crate::c_types::ThirtyTwoBytes, mut amount_msats: crate::c_types::derived::COption_u64Z, mut route_params_config: crate::lightning::routing::router::RouteParametersConfig, mut retry_strategy: crate::lightning::ln::outbound_payment::Retry) -> crate::c_types::derived::CResult_NoneBolt11PaymentErrorZ {
	let mut local_amount_msats = if amount_msats.is_some() { Some( { amount_msats.take() }) } else { None };
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.pay_for_bolt11_invoice(invoice.get_native_ref(), ::lightning::ln::channelmanager::PaymentId(payment_id.data), local_amount_msats, *unsafe { Box::from_raw(route_params_config.take_inner()) }, retry_strategy.into_native());
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::ln::outbound_payment::Bolt11PaymentError::native_into(e) }).into() };
	local_ret
}
...
#[no_mangle]
pub extern "C" fn ChannelManager_static_invoice_persisted(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut invoice_persisted_path: crate::lightning::onion_message::messenger::Responder) {
	unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.static_invoice_persisted(*unsafe { Box::from_raw(invoice_persisted_path.take_inner()) })
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_respond_to_static_invoice_request(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut invoice: crate::lightning::offers::static_invoice::StaticInvoice, mut responder: crate::lightning::onion_message::messenger::Responder, mut invoice_request: crate::lightning::offers::invoice_request::InvoiceRequest, mut invoice_request_path: crate::lightning::blinded_path::message::BlindedMessagePath) -> crate::c_types::derived::CResult_NoneBolt12SemanticErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.respond_to_static_invoice_request(*unsafe { Box::from_raw(invoice.take_inner()) }, *unsafe { Box::from_raw(responder.take_inner()) }, *unsafe { Box::from_raw(invoice_request.take_inner()) }, *unsafe { Box::from_raw(invoice_request_path.take_inner()) });
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_funding_transaction_generated_manual_broadcast(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut temporary_channel_id: crate::lightning::ln::types::ChannelId, mut counterparty_node_id: crate::c_types::PublicKey, mut funding_transaction: crate::c_types::Transaction) -> crate::c_types::derived::CResult_NoneAPIErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.funding_transaction_generated_manual_broadcast(*unsafe { Box::from_raw(temporary_channel_id.take_inner()) }, counterparty_node_id.into_rust(), funding_transaction.into_bitcoin());
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::util::errors::APIError::native_into(e) }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_funding_transaction_signed(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, channel_id: &crate::lightning::ln::types::ChannelId, mut counterparty_node_id: crate::c_types::PublicKey, mut transaction: crate::c_types::Transaction) -> crate::c_types::derived::CResult_NoneAPIErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.funding_transaction_signed(channel_id.get_native_ref(), &counterparty_node_id.into_rust(), transaction.into_bitcoin());
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::util::errors::APIError::native_into(e) }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_needs_pending_htlc_processing(this_arg: &crate::lightning::ln::channelmanager::ChannelManager) -> bool {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.needs_pending_htlc_processing();
	ret
}
...
pub extern "C" fn ChannelManager_accept_inbound_channel(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, temporary_channel_id: &crate::lightning::ln::types::ChannelId, mut counterparty_node_id: crate::c_types::PublicKey, mut user_channel_id: crate::c_types::U128, mut config_overrides: crate::lightning::util::config::ChannelConfigOverrides) -> crate::c_types::derived::CResult_NoneAPIErrorZ {
	let mut local_config_overrides = if config_overrides.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(config_overrides.take_inner()) } }) };
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.accept_inbound_channel(temporary_channel_id.get_native_ref(), &counterparty_node_id.into_rust(), user_channel_id.into(), local_config_overrides);
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::util::errors::APIError::native_into(e) }).into() };
	local_ret
}
...
pub extern "C" fn ChannelManager_accept_inbound_channel_from_trusted_peer_0conf(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, temporary_channel_id: &crate::lightning::ln::types::ChannelId, mut counterparty_node_id: crate::c_types::PublicKey, mut user_channel_id: crate::c_types::U128, mut config_overrides: crate::lightning::util::config::ChannelConfigOverrides) -> crate::c_types::derived::CResult_NoneAPIErrorZ {
	let mut local_config_overrides = if config_overrides.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(config_overrides.take_inner()) } }) };
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.accept_inbound_channel_from_trusted_peer_0conf(temporary_channel_id.get_native_ref(), &counterparty_node_id.into_rust(), user_channel_id.into(), local_config_overrides);
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::util::errors::APIError::native_into(e) }).into() };
	local_ret
}
...
pub extern "C" fn Bolt11InvoiceParameters_get_amount_msats(this_ptr: &Bolt11InvoiceParameters) -> crate::c_types::derived::COption_u64Z {
	let mut inner_val = &mut Bolt11InvoiceParameters::get_native_mut_ref(this_ptr).amount_msats;
	let mut local_inner_val = if inner_val.is_none() { crate::c_types::derived::COption_u64Z::None } else { crate::c_types::derived::COption_u64Z::Some( { inner_val.unwrap() }) };
	local_inner_val
}
/// The amount for the invoice, if any.
...
pub extern "C" fn Bolt11InvoiceParameters_get_description(this_ptr: &Bolt11InvoiceParameters) -> crate::lightning_invoice::Bolt11InvoiceDescription {
	let mut inner_val = &mut Bolt11InvoiceParameters::get_native_mut_ref(this_ptr).description;
...
pub extern "C" fn Bolt11InvoiceParameters_get_invoice_expiry_delta_secs(this_ptr: &Bolt11InvoiceParameters) -> crate::c_types::derived::COption_u32Z {
	let mut inner_val = &mut Bolt11InvoiceParameters::get_native_mut_ref(this_ptr).invoice_expiry_delta_secs;
...
pub extern "C" fn Bolt11InvoiceParameters_get_min_final_cltv_expiry_delta(this_ptr: &Bolt11InvoiceParameters) -> crate::c_types::derived::COption_u16Z {
	let mut inner_val = &mut Bolt11InvoiceParameters::get_native_mut_ref(this_ptr).min_final_cltv_expiry_delta;
...
pub extern "C" fn Bolt11InvoiceParameters_get_payment_hash(this_ptr: &Bolt11InvoiceParameters) -> crate::c_types::derived::COption_ThirtyTwoBytesZ {
	let mut inner_val = &mut Bolt11InvoiceParameters::get_native_mut_ref(this_ptr).payment_hash;
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_create_offer_builder_using_router(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut router: crate::lightning::onion_message::messenger::MessageRouter) -> crate::c_types::derived::CResult_OfferWithDerivedMetadataBuilderBolt12SemanticErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.create_offer_builder_using_router(router);
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { crate::lightning::offers::offer::OfferWithDerivedMetadataBuilder { inner: ObjOps::heap_alloc(o), is_owned: true } }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::offers::parse::Bolt12SemanticError::native_into(e) }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_create_refund_builder(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut amount_msats: u64, mut absolute_expiry: u64, mut payment_id: crate::c_types::ThirtyTwoBytes, mut retry_strategy: crate::lightning::ln::outbound_payment::Retry, mut route_params_config: crate::lightning::routing::router::RouteParametersConfig) -> crate::c_types::derived::CResult_RefundMaybeWithDerivedMetadataBuilderBolt12SemanticErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.create_refund_builder(amount_msats, core::time::Duration::from_secs(absolute_expiry), ::lightning::ln::channelmanager::PaymentId(payment_id.data), retry_strategy.into_native(), *unsafe { Box::from_raw(route_params_config.take_inner()) });
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_create_refund_builder_using_router(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut router: crate::lightning::onion_message::messenger::MessageRouter, mut amount_msats: u64, mut absolute_expiry: u64, mut payment_id: crate::c_types::ThirtyTwoBytes, mut retry_strategy: crate::lightning::ln::outbound_payment::Retry, mut route_params_config: crate::lightning::routing::router::RouteParametersConfig) -> crate::c_types::derived::CResult_RefundMaybeWithDerivedMetadataBuilderBolt12SemanticErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.create_refund_builder_using_router(router, amount_msats, core::time::Duration::from_secs(absolute_expiry), ::lightning::ln::channelmanager::PaymentId(payment_id.data), retry_strategy.into_native(), *unsafe { Box::from_raw(route_params_config.take_inner()) });
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { crate::lightning::offers::refund::RefundMaybeWithDerivedMetadataBuilder { inner: ObjOps::heap_alloc(o), is_owned: true } }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::offers::parse::Bolt12SemanticError::native_into(e) }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_get_async_receive_offer(this_arg: &crate::lightning::ln::channelmanager::ChannelManager) -> crate::c_types::derived::CResult_OfferNoneZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.get_async_receive_offer();
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { crate::lightning::offers::offer::Offer { inner: ObjOps::heap_alloc(o), is_owned: true } }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { () /*e*/ }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_set_paths_to_static_invoice_server(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut paths_to_static_invoice_server: crate::c_types::derived::CVec_BlindedMessagePathZ) -> crate::c_types::derived::CResult_NoneNoneZ {
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_pay_for_offer(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, offer: &crate::lightning::offers::offer::Offer, mut amount_msats: crate::c_types::derived::COption_u64Z, mut payment_id: crate::c_types::ThirtyTwoBytes, mut optional_params: crate::lightning::ln::channelmanager::OptionalOfferPaymentParams) -> crate::c_types::derived::CResult_NoneBolt12SemanticErrorZ {
	let mut local_amount_msats = if amount_msats.is_some() { Some( { amount_msats.take() }) } else { None };
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.pay_for_offer(offer.get_native_ref(), local_amount_msats, ::lightning::ln::channelmanager::PaymentId(payment_id.data), *unsafe { Box::from_raw(optional_params.take_inner()) });
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::offers::parse::Bolt12SemanticError::native_into(e) }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_pay_for_offer_from_hrn(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, offer: &crate::lightning::offers::offer::OfferFromHrn, mut amount_msats: u64, mut payment_id: crate::c_types::ThirtyTwoBytes, mut optional_params: crate::lightning::ln::channelmanager::OptionalOfferPaymentParams) -> crate::c_types::derived::CResult_NoneBolt12SemanticErrorZ {
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.pay_for_offer_from_hrn(offer.get_native_ref(), amount_msats, ::lightning::ln::channelmanager::PaymentId(payment_id.data), *unsafe { Box::from_raw(optional_params.take_inner()) });
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::offers::parse::Bolt12SemanticError::native_into(e) }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_pay_for_offer_with_quantity(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, offer: &crate::lightning::offers::offer::Offer, mut amount_msats: crate::c_types::derived::COption_u64Z, mut payment_id: crate::c_types::ThirtyTwoBytes, mut optional_params: crate::lightning::ln::channelmanager::OptionalOfferPaymentParams, mut quantity: u64) -> crate::c_types::derived::CResult_NoneBolt12SemanticErrorZ {
	let mut local_amount_msats = if amount_msats.is_some() { Some( { amount_msats.take() }) } else { None };
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.pay_for_offer_with_quantity(offer.get_native_ref(), local_amount_msats, ::lightning::ln::channelmanager::PaymentId(payment_id.data), *unsafe { Box::from_raw(optional_params.take_inner()) }, quantity);
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { crate::lightning::offers::parse::Bolt12SemanticError::native_into(e) }).into() };
	local_ret
}
...
pub extern "C" fn ChannelManager_pay_for_offer_from_human_readable_name(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut name: crate::lightning::onion_message::dns_resolution::HumanReadableName, mut amount_msats: u64, mut payment_id: crate::c_types::ThirtyTwoBytes, mut optional_params: crate::lightning::ln::channelmanager::OptionalOfferPaymentParams, mut dns_resolvers: crate::c_types::derived::CVec_DestinationZ) -> crate::c_types::derived::CResult_NoneNoneZ {
	...
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.pay_for_offer_from_human_readable_name(*unsafe { Box::from_raw(name.take_inner()) }, amount_msats, ::lightning::ln::channelmanager::PaymentId(payment_id.data), *unsafe { Box::from_raw(optional_params.take_inner()) }, local_dns_resolvers);
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { () /*e*/ }).into() };
	local_ret
}
...
#[must_use]
#[no_mangle]
pub extern "C" fn ChannelManager_blinded_paths_for_async_recipient(this_arg: &crate::lightning::ln::channelmanager::ChannelManager, mut recipient_id: crate::c_types::derived::CVec_u8Z, mut relative_expiry: crate::c_types::derived::COption_u64Z) -> crate::c_types::derived::CResult_CVec_BlindedMessagePathZNoneZ {
	...
	let mut ret = unsafe { &*ObjOps::untweak_ptr(this_arg.inner) }.blinded_paths_for_async_recipient(local_recipient_id, local_relative_expiry);
...
impl From<nativeChannelManager> for crate::lightning::ln::msgs::BaseMessageHandler {
	fn from(obj: nativeChannelManager) -> Self {
		...
		let mut ret = ChannelManager_as_BaseMessageHandler(&rust_obj);
		// We want to free rust_obj when ret gets drop()'d, not rust_obj, so forget it and set ret's free() fn
		core::mem::forget(rust_obj);
...
#[no_mangle]
pub extern "C" fn ChannelManager_as_BaseMessageHandler(this_arg: &ChannelManager) -> crate::lightning::ln::msgs::BaseMessageHandler {
	crate::lightning::ln::msgs::BaseMessageHandler {
		this_arg: unsafe { ObjOps::untweak_ptr((*this_arg).inner) as *mut c_void },
		free: None,
		get_and_clear_pending_msg_events: ChannelManager_BaseMessageHandler_get_and_clear_pending_msg_events,
		peer_disconnected: ChannelManager_BaseMessageHandler_peer_disconnected,
		provided_node_features: ChannelManager_BaseMessageHandler_provided_node_features,
		provided_init_features: ChannelManager_BaseMessageHandler_provided_init_features,
		peer_connected: ChannelManager_BaseMessageHandler_peer_connected,
	}
}

#[must_use]
extern "C" fn ChannelManager_BaseMessageHandler_get_and_clear_pending_msg_events(this_arg: *const c_void) -> crate::c_types::derived::CVec_MessageSendEventZ {
	let mut ret = <nativeChannelManager as lightning::ln::msgs::BaseMessageHandler>::get_and_clear_pending_msg_events(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, );
...
extern "C" fn ChannelManager_BaseMessageHandler_peer_disconnected(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey) {
	<nativeChannelManager as lightning::ln::msgs::BaseMessageHandler>::peer_disconnected(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust())
}
#[must_use]
extern "C" fn ChannelManager_BaseMessageHandler_provided_node_features(this_arg: *const c_void) -> crate::lightning_types::features::NodeFeatures {
	let mut ret = <nativeChannelManager as lightning::ln::msgs::BaseMessageHandler>::provided_node_features(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, );
	crate::lightning_types::features::NodeFeatures { inner: ObjOps::heap_alloc(ret), is_owned: true }
}
#[must_use]
extern "C" fn ChannelManager_BaseMessageHandler_provided_init_features(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey) -> crate::lightning_types::features::InitFeatures {
	let mut ret = <nativeChannelManager as lightning::ln::msgs::BaseMessageHandler>::provided_init_features(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust());
	crate::lightning_types::features::InitFeatures { inner: ObjOps::heap_alloc(ret), is_owned: true }
}
#[must_use]
extern "C" fn ChannelManager_BaseMessageHandler_peer_connected(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey, msg: &crate::lightning::ln::msgs::Init, mut inbound: bool) -> crate::c_types::derived::CResult_NoneNoneZ {
	let mut ret = <nativeChannelManager as lightning::ln::msgs::BaseMessageHandler>::peer_connected(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust(), msg.get_native_ref(), inbound);
	let mut local_ret = match ret { Ok(mut o) => crate::c_types::CResultTempl::ok( { () /*o*/ }).into(), Err(mut e) => crate::c_types::CResultTempl::err( { () /*e*/ }).into() };
	local_ret
}
...
pub extern "C" fn ChannelManager_as_Listen(this_arg: &ChannelManager) -> crate::lightning::chain::Listen {
	crate::lightning::chain::Listen {
		...
		blocks_disconnected: ChannelManager_Listen_blocks_disconnected,
	}
}
...
extern "C" fn ChannelManager_Listen_blocks_disconnected(this_arg: *const c_void, mut fork_point_block: crate::lightning::chain::BestBlock) {
	<nativeChannelManager as lightning::chain::Listen>::blocks_disconnected(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, *unsafe { Box::from_raw(fork_point_block.take_inner()) })
}
...
pub extern "C" fn ChannelManager_as_ChannelMessageHandler(this_arg: &ChannelManager) -> crate::lightning::ln::msgs::ChannelMessageHandler {
	crate::lightning::ln::msgs::ChannelMessageHandler {
		...
		handle_splice_init: ChannelManager_ChannelMessageHandler_handle_splice_init,
		...
		handle_commitment_signed_batch: ChannelManager_ChannelMessageHandler_handle_commitment_signed_batch,
...
extern "C" fn ChannelManager_ChannelMessageHandler_handle_peer_storage(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey, mut msg: crate::lightning::ln::msgs::PeerStorage) {
	<nativeChannelManager as lightning::ln::msgs::ChannelMessageHandler>::handle_peer_storage(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust(), *unsafe { Box::from_raw(msg.take_inner()) })
}
extern "C" fn ChannelManager_ChannelMessageHandler_handle_peer_storage_retrieval(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey, mut msg: crate::lightning::ln::msgs::PeerStorageRetrieval) {
	<nativeChannelManager as lightning::ln::msgs::ChannelMessageHandler>::handle_peer_storage_retrieval(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust(), *unsafe { Box::from_raw(msg.take_inner()) })
}
...
extern "C" fn ChannelManager_ChannelMessageHandler_handle_splice_init(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey, msg: &crate::lightning::ln::msgs::SpliceInit) {
	<nativeChannelManager as lightning::ln::msgs::ChannelMessageHandler>::handle_splice_init(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust(), msg.get_native_ref())
}
...
extern "C" fn ChannelManager_ChannelMessageHandler_handle_update_fulfill_htlc(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey, mut msg: crate::lightning::ln::msgs::UpdateFulfillHTLC) {
	<nativeChannelManager as lightning::ln::msgs::ChannelMessageHandler>::handle_update_fulfill_htlc(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust(), *unsafe { Box::from_raw(msg.take_inner()) })
}
...
extern "C" fn ChannelManager_ChannelMessageHandler_handle_commitment_signed_batch(this_arg: *const c_void, mut their_node_id: crate::c_types::PublicKey, mut channel_id: crate::lightning::ln::types::ChannelId, mut batch: crate::c_types::derived::CVec_CommitmentSignedZ) {
	let mut local_batch = Vec::new(); for mut item in batch.into_rust().drain(..) { local_batch.push( { *unsafe { Box::from_raw(item.take_inner()) } }); };
	<nativeChannelManager as lightning::ln::msgs::ChannelMessageHandler>::handle_commitment_signed_batch(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, their_node_id.into_rust(), *unsafe { Box::from_raw(channel_id.take_inner()) }, local_batch)
}
...
pub extern "C" fn ChannelManager_as_AsyncPaymentsMessageHandler(this_arg: &ChannelManager) -> crate::lightning::onion_message::async_payments::AsyncPaymentsMessageHandler {
	crate::lightning::onion_message::async_payments::AsyncPaymentsMessageHandler {
		this_arg: unsafe { ObjOps::untweak_ptr((*this_arg).inner) as *mut c_void },
		free: None,
		handle_offer_paths_request: ChannelManager_AsyncPaymentsMessageHandler_handle_offer_paths_request,
		handle_offer_paths: ChannelManager_AsyncPaymentsMessageHandler_handle_offer_paths,
		handle_serve_static_invoice: ChannelManager_AsyncPaymentsMessageHandler_handle_serve_static_invoice,
		handle_static_invoice_persisted: ChannelManager_AsyncPaymentsMessageHandler_handle_static_invoice_persisted,
...
#[must_use]
extern "C" fn ChannelManager_AsyncPaymentsMessageHandler_handle_offer_paths_request(this_arg: *const c_void, mut message: crate::lightning::onion_message::async_payments::OfferPathsRequest, mut context: crate::lightning::blinded_path::message::AsyncPaymentsContext, mut responder: crate::lightning::onion_message::messenger::Responder) -> crate::c_types::derived::COption_C2Tuple_OfferPathsResponseInstructionZZ {
	let mut local_responder = if responder.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(responder.take_inner()) } }) };
	let mut ret = <nativeChannelManager as lightning::onion_message::async_payments::AsyncPaymentsMessageHandler>::handle_offer_paths_request(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, *unsafe { Box::from_raw(message.take_inner()) }, context.into_native(), local_responder);
	let mut local_ret = if ret.is_none() { crate::c_types::derived::COption_C2Tuple_OfferPathsResponseInstructionZZ::None } else { crate::c_types::derived::COption_C2Tuple_OfferPathsResponseInstructionZZ::Some( { let (mut orig_ret_0_0, mut orig_ret_0_1) = (ret.unwrap()); let mut local_ret_0 = (crate::lightning::onion_message::async_payments::OfferPaths { inner: ObjOps::heap_alloc(orig_ret_0_0), is_owned: true }, crate::lightning::onion_message::messenger::ResponseInstruction { inner: ObjOps::heap_alloc(orig_ret_0_1), is_owned: true }).into(); local_ret_0 }) };
	local_ret
}
#[must_use]
extern "C" fn ChannelManager_AsyncPaymentsMessageHandler_handle_offer_paths(this_arg: *const c_void, mut message: crate::lightning::onion_message::async_payments::OfferPaths, mut context: crate::lightning::blinded_path::message::AsyncPaymentsContext, mut responder: crate::lightning::onion_message::messenger::Responder) -> crate::c_types::derived::COption_C2Tuple_ServeStaticInvoiceResponseInstructionZZ {
	let mut local_responder = if responder.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(responder.take_inner()) } }) };
	let mut ret = <nativeChannelManager as lightning::onion_message::async_payments::AsyncPaymentsMessageHandler>::handle_offer_paths(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, *unsafe { Box::from_raw(message.take_inner()) }, context.into_native(), local_responder);
	let mut local_ret = if ret.is_none() { crate::c_types::derived::COption_C2Tuple_ServeStaticInvoiceResponseInstructionZZ::None } else { crate::c_types::derived::COption_C2Tuple_ServeStaticInvoiceResponseInstructionZZ::Some( { let (mut orig_ret_0_0, mut orig_ret_0_1) = (ret.unwrap()); let mut local_ret_0 = (crate::lightning::onion_message::async_payments::ServeStaticInvoice { inner: ObjOps::heap_alloc(orig_ret_0_0), is_owned: true }, crate::lightning::onion_message::messenger::ResponseInstruction { inner: ObjOps::heap_alloc(orig_ret_0_1), is_owned: true }).into(); local_ret_0 }) };
	local_ret
}
extern "C" fn ChannelManager_AsyncPaymentsMessageHandler_handle_serve_static_invoice(this_arg: *const c_void, mut message: crate::lightning::onion_message::async_payments::ServeStaticInvoice, mut context: crate::lightning::blinded_path::message::AsyncPaymentsContext, mut responder: crate::lightning::onion_message::messenger::Responder) {
	let mut local_responder = if responder.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(responder.take_inner()) } }) };
	<nativeChannelManager as lightning::onion_message::async_payments::AsyncPaymentsMessageHandler>::handle_serve_static_invoice(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, *unsafe { Box::from_raw(message.take_inner()) }, context.into_native(), local_responder)
}
extern "C" fn ChannelManager_AsyncPaymentsMessageHandler_handle_static_invoice_persisted(this_arg: *const c_void, mut message: crate::lightning::onion_message::async_payments::StaticInvoicePersisted, mut context: crate::lightning::blinded_path::message::AsyncPaymentsContext) {
	<nativeChannelManager as lightning::onion_message::async_payments::AsyncPaymentsMessageHandler>::handle_static_invoice_persisted(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, *unsafe { Box::from_raw(message.take_inner()) }, context.into_native())
}
#[must_use]
extern "C" fn ChannelManager_AsyncPaymentsMessageHandler_handle_held_htlc_available(this_arg: *const c_void, mut message: crate::lightning::onion_message::async_payments::HeldHtlcAvailable, mut context: crate::lightning::blinded_path::message::AsyncPaymentsContext, mut responder: crate::lightning::onion_message::messenger::Responder) -> crate::c_types::derived::COption_C2Tuple_ReleaseHeldHtlcResponseInstructionZZ {
	let mut local_responder = if responder.inner.is_null() { None } else { Some( { *unsafe { Box::from_raw(responder.take_inner()) } }) };
	let mut ret = <nativeChannelManager as lightning::onion_message::async_payments::AsyncPaymentsMessageHandler>::handle_held_htlc_available(unsafe { &mut *(this_arg as *mut nativeChannelManager) }, *unsafe { Box::from_raw(message.take_inner()) }, context.into_native(), local_responder);
...
pub extern "C" fn ChannelManagerReadArgs_get_entropy_source(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::sign::EntropySource {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).entropy_source;
	inner_val
}
/// A cryptographically secure source of entropy.
#[no_mangle]
...
pub extern "C" fn ChannelManagerReadArgs_get_node_signer(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::sign::NodeSigner {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).node_signer;
	inner_val
}
/// A signer that is able to perform node-scoped cryptographic operations.
#[no_mangle]
...
pub extern "C" fn ChannelManagerReadArgs_get_signer_provider(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::sign::SignerProvider {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).signer_provider;
	inner_val
}
/// The keys provider which will give us relevant keys. Some keys will be loaded during
/// deserialization and [`SignerProvider::derive_channel_signer`] will be used to derive
...
pub extern "C" fn ChannelManagerReadArgs_get_fee_estimator(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::chain::chaininterface::FeeEstimator {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).fee_estimator;
	inner_val
}
/// The fee_estimator for use in the ChannelManager in the future.
///
...
pub extern "C" fn ChannelManagerReadArgs_get_chain_monitor(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::chain::Watch {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).chain_monitor;
	inner_val
}
/// The chain::Watch for use in the ChannelManager in the future.
///
...
pub extern "C" fn ChannelManagerReadArgs_get_tx_broadcaster(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::chain::chaininterface::BroadcasterInterface {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).tx_broadcaster;
	inner_val
}
/// The BroadcasterInterface which will be used in the ChannelManager in the future and may be
/// used to broadcast the latest local commitment transactions of channels which must be
...
pub extern "C" fn ChannelManagerReadArgs_get_router(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::routing::router::Router {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).router;
	inner_val
}
/// The router which will be used in the ChannelManager in the future for finding routes
/// on-the-fly for trampoline payments. Absent in private nodes that don't support forwarding.
...
pub extern "C" fn ChannelManagerReadArgs_get_message_router(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::onion_message::messenger::MessageRouter {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).message_router;
	inner_val
}
/// The [`MessageRouter`] used for constructing [`BlindedMessagePath`]s for [`Offer`]s,
/// [`Refund`]s, and any reply paths.
...
pub extern "C" fn ChannelManagerReadArgs_get_logger(this_ptr: &ChannelManagerReadArgs) -> *const crate::lightning::util::logger::Logger {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).logger;
	inner_val
}
/// The Logger for use in the ChannelManager and which may be used to log information during
/// deserialization.
...
#[no_mangle]
pub extern "C" fn ChannelManagerReadArgs_get_config(this_ptr: &ChannelManagerReadArgs) -> crate::lightning::util::config::UserConfig {
	let mut inner_val = &mut ChannelManagerReadArgs::get_native_mut_ref(this_ptr).config;
...
/// runtime settings which were stored when the ChannelManager was serialized.
#[no_mangle]
pub extern "C" fn ChannelManagerReadArgs_set_config(this_ptr: &mut ChannelManagerReadArgs, mut val: crate::lightning::util::config::UserConfig) {
	unsafe { &mut *ObjOps::untweak_ptr(this_ptr.inner) }.config = *unsafe { Box::from_raw(val.take_inner()) };
}
...
pub extern "C" fn ChannelManagerReadArgs_new(mut entropy_source: crate::lightning::sign::EntropySource, mut node_signer: crate::lightning::sign::NodeSigner, mut signer_provider: crate::lightning::sign::SignerProvider, mut fee_estimator: crate::lightning::chain::chaininterface::FeeEstimator, mut chain_monitor: crate::lightning::chain::Watch, mut tx_broadcaster: crate::lightning::chain::chaininterface::BroadcasterInterface, mut router: crate::lightning::routing::router::Router, mut message_router: crate::lightning::onion_message::messenger::MessageRouter, mut logger: crate::lightning::util::logger::Logger, mut config: crate::lightning::util::config::UserConfig, mut channel_monitors: crate::c_types::derived::CVec_ChannelMonitorZ) -> crate::lightning::ln::channelmanager::ChannelManagerReadArgs {
	...
	let mut ret = lightning::ln::channelmanager::ChannelManagerReadArgs::new(entropy_source, node_signer, signer_provider, fee_estimator, chain_monitor, tx_broadcaster, router, message_router, logger, *unsafe { Box::from_raw(config.take_inner()) }, local_channel_monitors);
